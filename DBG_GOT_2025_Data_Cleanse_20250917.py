...
def flag_invalid_states(df, logger, state_col='State'):
    ...
    if not invalid_states_df.empty:
        logger.warning(f"🏛️ Found {len(invalid_states_df)} rows with invalid state abbreviations")
    else:
...
if __name__ == "__main__":
    ...
    if latest_cleaned_file:
        ...
        logger.info("⚠️ No previously cleaned file found. Proceeding with original input file.")
