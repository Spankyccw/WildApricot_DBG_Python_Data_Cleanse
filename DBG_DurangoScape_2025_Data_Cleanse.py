# Title: DBG_DurangoScape_2025_Data_Cleanse
# Author: cwilliams
# Date: 2025/10/14
# Purpose: Clean Durango Botanic Gardens "DurangoScape" 2025 data before loading into Wild Apricot CMS
# Dependencies: pandas, openpyxl, xlrd
# Date/Name/Change
# 10/14/2025 cwilliams - Initial version adapted from DBG_GOT_2025_Data_Cleanse
...
# File paths
file_path1 = r'C:\Users\Charl\OneDrive\Documents\Development\Python\DBG\2025 DurangoScape ccw.xls'
output_dir2 = r'C:\Users\Charl\OneDrive\Documents\Development\Python\DBG'
base_name2 = "DurangoScape_2025_Attendance_ccw_clean"
...
log_filename = f"DBG_DurangoScape_2025_data_cleanse_{datetime_stamp}.log"
...
def validate_durangoscape_column(df, logger, column_name='DurangoScape 2025', expected_value='Yes'):
    if column_name not in df.columns:
        logger.warning(f"Column '{column_name}' not found")
        return {'invalid_count': 0, 'empty_count': 0, 'valid_count': 0}
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting DBG DurangoScape 2025 data cleaning process")
    ...
    try:
        if file_path1.endswith('.xls'):
            df1 = pd.read_excel(file_path1, engine='xlrd')
            logger.info(f'Input File Loaded (.xls format): {file_path1}')
        else:
            df1 = pd.read_excel(file_path1, engine='openpyxl')
            logger.info(f'Input File Loaded (.xlsx format): {file_path1}')
    ...
    durangoscape_stats = validate_durangoscape_column(df1, logger, 'DurangoScape 2025', 'Yes')
    ...
    if not record_count_valid:
        ...
        try:
            df1.to_excel(full_path2, index=False, engine='openpyxl')
    ...
    logger.info(f"   - DurangoScape 2025 valid entries: {durangoscape_stats.get('valid_count', 0)}")
    logger.info(f"   - DurangoScape 2025 invalid/empty: {durangoscape_stats.get('invalid_count', 0) + durangoscape_stats.get('empty_count', 0)}")

    logger.info("DBG DurangoScape 2025 data cleaning process completed")
